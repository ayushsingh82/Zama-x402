...
export default function DashboardPage() {
  ...
  const transferCount = history.filter((h) => h.scheme === 'fhe-transfer').length;
  const shieldedCount = history.filter((h) => h.scheme === 'fhe-shielded-pool').length;
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-sm text-black/60 font-semibold">
          Pay-per-use x402 services backed by ERC7984 confidential tokens on Zama&apos;s FHEVM.
        </p>
      </div>

      {!isConnected && (
        <div className="bg-white border-2 border-black border-dashed rounded-xl p-6">
          <p className="text-sm font-bold text-black">Connect your wallet from the sidebar to pay for a service below.</p>
        </div>
      )}

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatTile label="Services" value={String(SERVICES.length)} sub="available now" />
        <StatTile label="fhe-transfer payments" value={String(transferCount)} sub="this browser" />
        <StatTile label="shielded-pool payments" value={String(shieldedCount)} sub="this browser" />
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {SERVICES.map((service) => (
            ...
            >
              <span className="self-start text-xs font-black px-3 py-1 rounded-full bg-red-500 text-white mb-3">
                {service.scheme}
              </span>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs mb-4">
                <div className="bg-gray-50 border-2 border-black rounded-lg px-3 py-2">
                  <p className="text-black/50 mb-0.5">Price</p>
                  <p className="font-mono font-bold text-black">{formatTokenAmount(service.amountRaw)} tokens</p>
                </div>
                <div className="bg-gray-50 border-2 border-black rounded-lg px-3 py-2">
                  <p className="text-black/50 mb-0.5">Endpoint</p>
                  <p className="font-mono font-bold text-black truncate">{service.endpoint}</p>
                </div>
              </div>
              <Link href="/test">
